...
const Services = () => {
  return (
    <section className="bg-[var(--bg)] py-12 px-4 md:px-20 font-serif">
      <div className="container mx-auto text-center">
        <h2 className="text-[var(--accent)] text-3xl font-bold mb-4">Projects</h2>
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {services.map((service, index) => (
            <div 
              key={index} 
              className="group bg-[var(--surface)] border border-[var(--accent-2)] shadow-2xl rounded-2xl overflow-hidden w-80 transform transition-all duration-500 hover:scale-100 hover:shadow-3xl hover:-translate-y-2 flex flex-col justify-between"
            >
              <div className="relative">
                <div className='flex items-center space-x-4 p-6 pb-4 relative z-20'> 
                  <div className="w-14 h-14 bg-[var(--accent-2)] rounded-full flex items-center justify-center text-white shadow-lg group-hover:scale-100 transition-transform duration-300">
                    <FontAwesomeIcon icon={service.icon} className="text-2xl" />
                  </div>
                  <h3 className="text-xl font-bold text-[var(--text)] transition-colors duration-300">{service.title}</h3>
                </div>
                ...
                <p className="text-[var(--text)] mb-6 px-6 leading-relaxed transition-colors duration-300">{service.description}</p>
              </div>
